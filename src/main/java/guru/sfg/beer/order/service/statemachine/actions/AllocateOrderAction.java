...
public class AllocateOrderAction implements Action<BeerOrderStatusEnum, BeerOrderEventsEnum> {
    ...
    public void execute(StateContext<BeerOrderStatusEnum, BeerOrderEventsEnum> stateContext) {
        ...
        jmsTemplate.convertAndSend(JmsConfig.ALLOCATE_ORDER_QUEUE, AllocateOrderRequest.builder().beerOrder(beerOrderMapper.beerOrderToDto(beerOrder)));

        log.debug("Sent validation request for order ID " + beerOrderId);
    }
}
